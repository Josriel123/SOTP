...
APantheonGameMode::APantheonGameMode() {
  ...
  HMS_MigrationSettings.CreateGameSaveUpdateParams.IgnoreHostPawn =
      false; // keep the host pawn
}

void APantheonGameMode::InitGame(const FString &MapName, const FString &Options,
                                 FString &Error) {
  Super::InitGame(MapName, Options, Error);

  UE_LOG(LogTemp, Warning, TEXT("InitGame ? Map: %s  Options: %s"), *MapName,
         *Options);
  ...
  UE_LOG(LogPantheonGameMode, Log,
         TEXT("InitGame ? Expecting %d human players"), ExpectedHumans);
...
void APantheonGameMode::PostLogin(APlayerController *NewPlayer) {
  // Call bot-replacement BEFORE Super so the engine doesn't spawn a new pawn
  // yet
  ...
  UE_LOG(LogPantheonGameMode, Log, TEXT("PostLogin ? Humans=%d / %d"),
         HumansNow, ExpectedHumans);
}

void APantheonGameMode::StartMatch() {
  UE_LOG(LogPantheonGameMode, Log,
         TEXT("=== StartMatch() called on server ==="));
  ...
  HMS_StartAsyncCreateGameSaveTask(
      HMS_MigrationSettings.CreateGameSaveUpdateParams);
...
void APantheonGameMode::HMS_OnGameRehosted_Cpp() {
  ...
  UE_LOG(LogPantheonGameMode, Log,
         TEXT("[HMS_OnGameRehosted_Cpp] Server repairing state after host "
              "migration"));
  ...
  for (TActorIterator<APawn> It(GetWorld()); It; ++It) {
    ...
    if (P->GetController() == nullptr) {
      ...
      TSubclassOf<AAIController> BotControllerClassToUse =
      ...
      AAIController *NewBot = GetWorld()->SpawnActor<AAIController>(
          BotControllerClassToUse, P->GetActorLocation(),
          P->GetActorRotation());

      if (NewBot) {
        NewBot->Possess(P);
        UE_LOG(
            LogPantheonGameMode, Log,
            TEXT("[HMS_OnGameRehosted_Cpp] Repossessed orphan pawn %s with %s"),
...
void APantheonGameMode::EnsureBotPlayerState(AAIController *AI) {
  ...
  if (AI->PlayerState == nullptr) {
    AI->InitPlayerState(); // gives us an APantheonPlayerState (based on
                           // GameMode's PlayerStateClass)
  }
...
void APantheonGameMode::FillWithBots() {
  ...
  UE_LOG(LogPantheonGameMode, Log,
         TEXT("[FillWithBots] Humans=%d  BotsToSpawn=%d"), Humans, Needed);

  for (int32 i = 0; i < Needed; ++i) {
    APantheonBotController *Bot =
        World->SpawnActor<APantheonBotController>(BotControllerClass);
    if (!Bot) {
      UE_LOG(LogPantheonGameMode, Warning,
             TEXT("  !! Failed to spawn APantheonBotController [%d]"), i);
      continue;
    }
    ...
    if (!IsValid(Start)) {
      UE_LOG(LogPantheonGameMode, Warning,
             TEXT("No valid PlayerStart – skipping bot"));
      continue;
    }
    ...
    if (!PawnClass) {
      UE_LOG(LogPantheonGameMode, Error,
             TEXT("  !! PawnClass is NULL for Bot_%02d"), i + 1);
      continue;
    }
    ...
    APawn *Pawn = World->SpawnActor<APawn>(PawnClass, Start->GetActorLocation(),
                                           Start->GetActorRotation(), Params);

    if (!Pawn) {
      UE_LOG(LogPantheonGameMode, Error,
             TEXT("  !! FAILED to spawn pawn %s for Bot_%02d"),
    ...
    UE_LOG(LogPantheonGameMode, Log,
           TEXT("[FillWithBots] %s spawned  UniqueId=%s"), *PS->GetPlayerName(),
           *IdStr);
  }
}
...
void APantheonGameMode::AssignRoles() {
  ...
  for (APlayerState *State : GameState->PlayerArray) {
    ...
    UE_LOG(LogPantheonGameMode, Log,
           ...
           *PS->GetPlayerName(), *PS->ChosenRole, *RowKey.ToString(), PawnName);
  }
  UE_LOG(LogPantheonGameMode, Log, TEXT("---------------------------"));
}

void APantheonGameMode::ReplaceBotIfPossible(APlayerController *JoiningPC) {
  if (!HasAuthority() || !IsValid(JoiningPC) ||
      GetMatchState() != MatchState::InProgress)
    return;

  APantheonPlayerState *HumanPS =
      Cast<APantheonPlayerState>(JoiningPC->PlayerState);
  if (!HumanPS)
    return;

  APantheonBotController *ChosenBot = nullptr;

  // Prefer a bot that is controlling a pawn that used to be human (has a
  // "UniqueId=" tag)
  ...
  if (DesiredRole.IsEmpty()) {
    if (APantheonPlayerState *BotPS =
            Cast<APantheonPlayerState>(ChosenBot->PlayerState))
      DesiredRole =
          BotPS->ChosenRole.IsEmpty() ? TEXT("Survivor") : BotPS->ChosenRole;
    else
      DesiredRole = TEXT("Survivor");
  }

  TSubclassOf<APawn> DesiredClass =
      HumanPS->GetChosenPawnClassForRole(DesiredRole);
  if (!DesiredClass)
    DesiredClass = BotPawn->GetClass();
...
void APantheonGameMode::SpawnReplacementBotAt(const FTransform &Xf,
  ...
  if (!EffectivePawnClass) {
    UE_LOG(LogPantheonGameMode, Warning,
           TEXT("[SpawnReplacementBotAt] No valid pawn class to spawn."));
    return;
  }
  ...
  CParams.SpawnCollisionHandlingOverride =
      ESpawnActorCollisionHandlingMethod::AlwaysSpawn;

  AAIController *Bot =
      World->SpawnActor<AAIController>(EffectiveAIControllerClass, Xf, CParams);
  if (!Bot) {
    UE_LOG(LogPantheonGameMode, Warning,
           TEXT("[SpawnReplacementBotAt] Failed to spawn bot controller."));
    return;
  }
  ...
  if (!NewPawn) {
    UE_LOG(LogPantheonGameMode, Warning,
           TEXT("[SpawnReplacementBotAt] Failed to spawn pawn for bot. "
  ...
  UE_LOG(LogPantheonGameMode, Log,
         TEXT("[SpawnReplacementBotAt] Spawned replacement bot %s with pawn %s "
              "at X=%.3f Y=%.3f Z=%.3f"),
         *GetNameSafe(Bot), *GetNameSafe(NewPawn),
         NewPawn->GetActorLocation().X, NewPawn->GetActorLocation().Y,
         NewPawn->GetActorLocation().Z);
}

AAIController *APantheonGameMode::SpawnBotControllerAndPossessExisting(
    APawn *ExistingPawn, const FTransform &DesiredTransform) {
  if (!HasAuthority() || !ExistingPawn)
    return nullptr;
  ...
  ExistingPawn->SetActorTransform(DesiredTransform, false, nullptr,
                                  ETeleportType::TeleportPhysics);
  ...
  AAIController *NewBot = World->SpawnActor<AAIController>(
      EffectiveAIControllerClass, DesiredTransform, CParams);
  if (!NewBot) {
    UE_LOG(LogPantheonGameMode, Warning,
           TEXT("[SpawnBotControllerAndPossessExisting] Failed to spawn bot "
                "controller."));
    return nullptr;
  }
  ...
  UE_LOG(LogPantheonGameMode, Log,
         TEXT("[SpawnBotControllerAndPossessExisting] Bot %s took over pawn %s "
              "at X=%.3f Y=%.3f Z=%.3f"),
         *GetNameSafe(NewBot), *GetNameSafe(ExistingPawn),
         ExistingPawn->GetActorLocation().X, ExistingPawn->GetActorLocation().Y,
...
AActor *
APantheonGameMode::ChoosePlayerStart_Implementation(AController *Player) {
...
void APantheonGameMode::HandleMatchIsWaitingToStart() {
  ...
  const int32 HumansNow = GameState->PlayerArray.Num();
  UE_LOG(LogPantheonGameMode, Log,
         TEXT("HandleMatchIsWaitingToStart ? Humans=%d / %d"), HumansNow,
         ExpectedHumans);
  ...
  if (HumansNow >= ExpectedHumans) {
    UE_LOG(
        LogPantheonGameMode, Log,
        TEXT(
            "All humans present – StartMatch() (HandleMatchIsWaitingToStart)"));
...
AHMS_PlayerController *APantheonGameMode::HMS_PickNewHost_Implementation() {
  ...
  for (APlayerState *PS : GameState->PlayerArray) {
    ...
    if (!PC)
      continue; // bot
    if (PC->NetConnection == nullptr)
      continue; // current host
    Eligible.Add(PS);
  }
  if (Eligible.Num() == 0)
    return nullptr;
...
void APantheonGameMode::RestartPlayer(AController *NewPlayer) {
  if (SkipNextRestart.Contains(NewPlayer)) {
    ...
    UE_LOG(LogPantheonGameMode, Log,
           TEXT("[RestartPlayer] Skipping due to bot replacement for %s"),
           *GetNameSafe(NewPlayer));
    return;
  }

  const bool bAlreadyHasPawn = NewPlayer && NewPlayer->GetPawn();
  UE_LOG(LogPantheonGameMode, Log, TEXT("RestartPlayer ? PawnIsNull=%s"),
         bAlreadyHasPawn ? TEXT("false") : TEXT("true"));

  // A) If this controller just replaced a bot it already owns a pawn.
  if (bAlreadyHasPawn) {
    if (APantheonCharacterBase *Char =
            Cast<APantheonCharacterBase>(NewPlayer->GetPawn())) {
      Char->LastHumanOwnerPS = NewPlayer->PlayerState;
      Char->LastHumanUniqueId = NewPlayer->PlayerState->GetUniqueId();

      if (APantheonPlayerState *PS =
              Cast<APantheonPlayerState>(NewPlayer->PlayerState)) {
  ...
  if (!HMS_MigrationFlags.IsGameRehosted ||
    ...
    if (!CurrentPawn || !CurrentPawn->IsA(PS->SelectedPawnClass)) {
      ...
      APawn *Final = GetWorld()->SpawnActor<APawn>(PS->SelectedPawnClass,
                                                   SpawnLoc, SpawnRot, P);
      ...
      if (IsValid(Final)) {
        ...
        UE_LOG(LogPantheonGameMode, Log,
               ...
               *Final->GetName(), *RowKey.ToString(), *NewPlayer->GetName());
      } else {
        UE_LOG(LogPantheonGameMode, Error,
...
void APantheonGameMode::Logout(AController *Exiting) {
  ...
  UE_LOG(LogPantheonGameMode, Log, TEXT("[Logout] %s leaving"),
         *GetNameSafe(Exiting));

  const bool bWasHuman = Cast<APlayerController>(Exiting) != nullptr;
  APantheonPlayerState *PS =
      Exiting ? Exiting->GetPlayerState<APantheonPlayerState>() : nullptr;

  // Try to get (or find) the pawn this controller was using
  APawn *LeavingPawn = Exiting ? Exiting->GetPawn() : nullptr;

  if (!LeavingPawn && PS) {
    const FName TagToFind(
        *FString::Printf(TEXT("UniqueId=%s"), *PS->GetPlayerName()));
    for (TActorIterator<APawn> It(GetWorld()); It; ++It) {
      APawn *P = *It;
      if (P && !P->IsPendingKillPending() && P->GetController() == nullptr &&
          P->Tags.Contains(TagToFind)) {
        LeavingPawn = P;
        break;
      }
    }
  ...
  if (bWasHuman && LeavingPawn && !LeavingPawn->IsPendingKillPending()) {
    ...
    // IMPORTANT: put an APantheonBotController (not base AAIController) back on
    // THIS pawn.
    const FTransform Xf = LeavingPawn->GetActorTransform();
    if (AAIController *NewBot =
            SpawnBotControllerAndPossessExisting(LeavingPawn, Xf)) {
      UE_LOG(LogPantheonGameMode, Log,
             TEXT("[Logout] Bot %s took over pawn %s at %s"),
             *GetNameSafe(NewBot), *GetNameSafe(LeavingPawn),
             *LeavingPawn->GetActorLocation().ToCompactString());
    } else {
      UE_LOG(LogPantheonGameMode, Warning,
             TEXT("[Logout] Failed to spawn bot controller for leaving pawn"));
    }
...
void APantheonGameMode::TagPawnWithPlayer(APawn *Pawn,
                                          APantheonPlayerState *PS) {
  if (!Pawn || !PS)
    return;
...
void APantheonGameMode::RememberLeavingPawn(AController *PC, APawn *Pawn,
                                            const FTransform &Xform,
                                            const FVector &Vel) {
  if (!HasAuthority() || !PC || !Pawn)
    return;
