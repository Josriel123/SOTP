...
void APantheonPlayerController::ServerSetReady_Implementation(bool bNewReady) {
  if (APantheonPlayerState *PS = GetPlayerState<APantheonPlayerState>()) {
    if (PS->IsReady() != bNewReady) {
      PS->SetReady(bNewReady); // write a simple setter that sets bIsReady and
                               // calls OnRep_IsReady on server
    }
  }
}
...
void APantheonPlayerController::Client_SaveLocalProfile_Implementation(
    const FPlayerProfileData &InProfile) {
  if (UPantheonGameInstance *GI = GetGameInstance<UPantheonGameInstance>()) {
...
void APantheonPlayerController::PawnLeavingGame() {
  ...
  if (GetLocalRole() == ROLE_Authority) {
    ...
    if (P) {
      ...
      if (const ACharacter *C = Cast<ACharacter>(P)) {
        if (const UCharacterMovementComponent *Move =
                C->GetCharacterMovement()) {
          Vel = Move->Velocity;
        }
      }

      if (APantheonGameMode *GM =
              GetWorld()->GetAuthGameMode<APantheonGameMode>()) {
        // You already have this helper in your GameMode; if yours only takes
        // (PC,Pawn,Transform), just drop "Vel" from the call and store only the
        // transform.
        GM->RememberLeavingPawn(this, P, Tf /*, Vel */);
      }

      // Keep the pawn alive: unpossess but DON'T call Super (it destroys the
      // pawn)
      UnPossess();
  ...
  // Super destroys the pawn by default, which is exactly what we want to avoid.
